        ...
        <div className="min-w-0 flex-1">
          ...
          <main className="relative min-h-[calc(100vh-68px)] px-4 py-5 sm:px-6 lg:px-8 overflow-hidden">
            ...
            <div className="relative z-10 mx-auto max-w-[1560px]">
              ...
              {activeView === "Scenario Simulator" && <SimulatorPage scenario={scenario} setScenario={setScenario} simulating={simulating} simulationRun={simulationRun} onRun={runSimulation} />}
...
function CommandCenter({ onView }: { onView: (view: View) => void }) {
  ...
  return <div className="pb-10"><PageHeader eyebrow="Command Center / Live" title="Hospital Command Center" subtitle="Predictive capacity intelligence for proactive hospital operations" action={<div className="flex items-center gap-3"><span className="flex items-center gap-2 rounded-full border border-command-red/25 bg-command-red/10 px-3 py-1.5 text-[10px] font-semibold tracking-[0.14em] text-command-red"><span className="status-pulse size-1.5 rounded-full bg-command-red" />LIVE</span><span className="hidden text-[10px] text-muted-foreground sm:inline">Updated {hospital.lastUpdated}</span></div>} />
    ...
    <InsightCard onSimulator={() => onView("Scenario Simulator")} />
  </div>;
}
...
function ForecastChart({ compact = false, range = "6H" }: { compact?: boolean; range?: string }) {
  const data = useMemo(() => api.getCommandCenter().forecast.map((point) => ({ ...point, highBand: point.high, lowBand: point.low })), []);
...
function ResourceStrip() { return <Panel title="Resource intelligence" meta="Current utilization across critical hospital resources" action={<Button variant="ghost" size="sm" className="text-[10px] text-command-cyan">View all <ArrowRight size={13} /></Button>}><div className="grid grid-cols-2 divide-x divide-y divide-command-border/60 sm:grid-cols-3 sm:divide-y-0 lg:grid-cols-6">{resources.map((resource) => <div key={resource.name} className="group p-3.5 transition-colors hover:bg-command-cyan/5"><div className="flex items-center justify-between"><ResourceIcon icon={resource.icon} /><RiskBadge risk={resource.risk} /></div><div className="mt-3 truncate text-[11px] font-semibold text-foreground">{resource.name}</div><div className="mt-2 flex items-end justify-between"><span className="mono-data text-xl font-semibold text-foreground">{resource.utilization}%</span><span className={cn("flex items-center text-[9px]", resource.trend.startsWith("-") ? "text-command-green" : "text-command-red")} >{resource.trend.startsWith("-") ? <TrendingDown size={11} /> : <TrendingUp size={11} />}{resource.trend}</span></div><div className="mt-2"><MiniSparkline values={resource.sparkline} tone={resource.risk === "CRITICAL" ? "red" : resource.risk === "LOW" ? "green" : "cyan"} /></div><div className="mt-1 text-[9px] text-muted-foreground">{resource.capacity}</div></div>)}</div></Panel>; }
...
function ProceduresPage() { return <div className="pb-10"><PageHeader eyebrow="Act / Schedule impact" title="Procedure Capacity" subtitle="See how scheduled procedures change downstream capacity." action={<Button size="sm" className="bg-command-cyan text-primary-foreground text-[10px]"><ClipboardList size={13} /> Add procedure</Button>} /><Panel title="Today's procedure load" meta="Thursday, September 10 · 08:00–20:00"><div className="overflow-x-auto p-5"><div className="min-w-[750px]"><div className="relative ml-28 h-8 border-b border-command-border">{["08:00", "10:00", "12:00", "14:00", "16:00", "18:00", "20:00"].map((time, index) => <span key={time} className="absolute -translate-x-1/2 text-[9px] text-muted-foreground" style={{ left: `${index * 16.66}%` }}>{time}</span>)}</div><div className="mt-4 space-y-4">{[["Cardiac procedures", 5, 25, "bg-command-red/60", "CAPACITY CONFLICT"], ["Orthopedic block", 30, 20, "bg-command-cyan/55", "ON TRACK"], ["General surgery", 52, 25, "bg-command-amber/60", "WATCH"], ["Endoscopy", 76, 13, "bg-command-green/55", "ON TRACK"]].map(([label, left, width, color, status]) => <div key={label} className="grid grid-cols-[100px_1fr] items-center gap-3"><div className="text-right text-[10px] text-foreground">{label}</div><div className="relative h-10 rounded bg-command/50"><div className={cn("absolute top-1 h-8 rounded border border-foreground/10 px-2 py-2 text-[9px] font-medium text-foreground", color)} style={{ left: `${left}%`, width: `${width}%` }}>{status}</div></div></div>)}</div></div></div></Panel><div className="mt-5 rounded-xl border border-command-amber/25 bg-command-amber/5 p-5"><div className="flex items-start gap-3"><AlertTriangle size={18} className="mt-0.5 text-command-amber" /><div><div className="text-[11px] font-semibold text-command-amber">Capacity conflict detected</div><p className="mt-1 text-[11px] leading-5 text-muted-foreground">14:00 cardiac procedures may create ICU capacity pressure at 17:00. The forecast engine recommends reviewing 2 elective cases.</p></div><Button variant="outline" size="sm" className="ml-auto hidden border-command-amber/30 text-[10px] text-command-amber sm:inline-flex">Review schedule</Button></div></div></div>; }
